...
(async function () {
  ...
  // ── 6. Build the language switcher UI ────────────────────
  const LANGUAGES = [
  ...
  function buildSwitcher(container) {
    ...
    // i18n.js may not have finished detecting yet — re-read from storage
    let activeLang = "en";
    ...
    btn?.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); window.toggleLangDropdown(); }
    });
  }

  // ── 7. Expose globals so i18n.js can call them too ───────
  ...
  // Close dropdown on outside click (shared)
  document.addEventListener("click", (e) => {
  ...
  window.setLanguage = async function (lang) {
    ...
    await window.i18n.setLanguage(lang);     // i18n.js handles storage + static translation
    // Rebuild switcher to reflect new selection
    const header = document.querySelector("header");
    if (header) buildSwitcher(header);
  };
